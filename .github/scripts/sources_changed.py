...
Either way, regenerates sync.yml's dropdown afterward so it always matches
the current sources.yaml. The actual git commit/push happens in the calling
workflow, same pattern as sync.yml's own "commit if changed" step -- this
script just leaves the working tree in the right state.
...
def main():
    ...
    print("\n=== Regenerating sync.yml dropdown ===")
    subprocess.run(
        [sys.executable, os.path.join(SCRIPTS_DIR, "update_sync_dropdown.py")],
        check=True,
    )
